...
# Recommendation: Start with Recursive-Landing
  ...
  Reasoning:
  - 🎯 Thanksgiving deadline - landing features are what you need for family
  playlists
  - ✅ Quick win first - CSV export (30-45 min) gives immediate value
  - 🔄 Then tackle harder ones - transparent layer & calendar while I have context       
  - ⏰ Creator fixes can wait - they're quality-of-life improvements, not launch
  blockers
...
### Task 1.1: Fix Import Append Behavior ⏳
**Priority:** HIGH
...
setItems(newItems); // ❌ REPLACES all items
```

**Desired Behavior:**
```javascript
// Should append:
setItems([...items, ...newItems]); // ✅ ADDS to existing items
```
...
**Success Criteria:**
- [ ] Import YouTube playlist with existing items → new videos added to end
- [ ] Import Drive folder with existing items → new images added to end
- [ ] Position numbers increment correctly
- [ ] No duplicate items

---

### Task 1.2: Improve License Note Wording ⏳
**Priority:** MEDIUM
...
**Key Changes:**
- ✅ Clarifies "content NOT from YouTube" is under CC BY-SA
- ✅ Mentions "specifically attributed" content exceptions
- ✅ States assumption: no attribution = CC BY-SA
...
#### Task 2.1: Add Transparent Layer (Prevent YouTube Clicks) ⏳
**Priority:** HIGH
...
#### Task 2.2: Add CSV Export Button ("See the Program") ⏳
**Priority:** HIGH
...
#### Task 2.3: Add Calendar .ics Generation ⏳
**Priority:** HIGH
...
document.getElementById('calendar-download').addEventListener('click', () => {
  ...
  const title = `Family Art Ritual – ${currentContent.title || 'Playlist'}`;
...
#### Task 2.4: Import from YouTube (Local Render) ⏳
**Priority:** MEDIUM
...
<button id="import-playlist-btn" class="btn-primary">
  🎬 Try Importing a YouTube Playlist
</button>
...
async function loadContent() {
  ...
  if (isLocal) {
    ...
    try {
      ...
      console.log('✅ Loaded local playlist:', currentItems.length, 'items');
...
function showLocalNotice() {
  ...
  `;
  notice.innerHTML = `
    💡 This is a temporary preview (stored in your browser).
...
- [ ] Paste playlist URL → fetches from creator API
...
- ✅ Instant preview
- ✅ Gateway to account creation
- ✅ Reuses existing API (no duplication)
- ✅ Works offline once imported

---

#### Task 2.5: Email Collector CTA ⏳
...
#### Task 2.6: Redesign YouTube Source Attribution ⏳
...
#### Task 2.7: Fix CSV Export Titles ⏳
**Priority:** HIGH
...
- ✅ Investigated: Titles are NOT being saved to Supabase by recursive-creator
- ❌ When importing YouTube playlists/videos, only video IDs are saved, not titles
- 🔧 **Fix needed in recursive-creator FIRST**: Modify import handlers to save YouTube titles to `items[].title` field
- 📋 Once titles are saved in DB, CSV export will work automatically (already reads `item.title`)
...
#### Task 2.8: Google Calendar Direct Integration (Research + Optional Implementation) ⏳
...
function addToGoogleCalendar() {
  const title = encodeURIComponent('Family Art Ritual – ' + currentContent.title);
...
- ✅ No OAuth required
- ✅ Works instantly
- ✅ User stays in control (can review before adding)
- ✅ Works on mobile

**Cons:**
- ❌ Google Calendar only (not Apple/Outlook)
- ❌ Requires Google account + being logged in
- ❌ Opens new tab (might get blocked by popup blockers)
...
### Task 1.3: Add YouTube Kids URL Support (Recursive Creator) ⏳
**Priority:** MEDIUM
**Time Estimate:** 30-60 min
**File:** `recursive-creator/app/dashboard/sequences/new/page.tsx`
...
### ✅ COMPLETED (Initial Implementation - Nov 23)

1. ✅ Task 2.2: CSV export button (30-45 min)
2. ✅ Task 2.1: Transparent layer for YouTube (1-2 hrs) - **REDESIGNED** (see Task 2.6)
3. ✅ Task 2.3: Calendar .ics generation (1.5-2 hrs)
4. ✅ Task 2.6: Redesigned YouTube source attribution (30-45 min) - Popup modal on click
5. ✅ Task 2.8: Google Calendar direct integration (30 min + 1-2 hrs)
6. ✅ Smart Previous button - Restart video before going to previous item
7. ✅ Fixed button positioning - Previous/Next always in same position
8. ✅ Image click navigation - Controls visible, black background clickable

---

### 🔴 HIGH PRIORITY (Fix User Feedback Issues)

**Phase 2B: Recursive Landing Fixes**
**Total Time:** ~1.5-2.5 hours

4. ⏳ **Task 2.6: Redesign YouTube Source Attribution** (30-45 min)
   - Remove always-visible source display
   - Add clickable transparent layer with popup
   - Test controls still work

5. ⏳ **Task 2.7: Fix CSV Export Titles** (20-30 min if titles in Supabase, 1-2 hrs if need API)
...
6. ⏳ **Task 2.8: Google Calendar Direct Integration** (30 min research + 1-2 hrs implementation)
   - Research Google Calendar URL scheme
   - Add "Add to Google Calendar" button
   - Keep .ics download for other calendar apps

7. ⏳ **Task 1.3: YouTube Kids URL Support** (30-60 min)
...
### 🔵 FUTURE ENHANCEMENTS (After Recursive-Creator Fixes)

**Phase 2D: Video Progress Features**
**Total Time:** ~2-3 hours
**Dependencies:** Requires Task 2.7 completion (recursive-creator saves titles + duration to DB)

**Task 2.9: Show Video Duration** (30 min)
- Display total video duration from DB: "Duration: 5:32"
- Show near video controls
- Long-term solution (no API calls after initial load)
- Implement after recursive-creator saves duration to Supabase

**Task 2.10: YouTube-Style Progress Bar** (1.5-2 hrs)
- Visual progress bar (like YouTube) instead of clock timer
- Gray background bar = 100% (total duration from DB)
- Red fill bar = current progress (updates as video plays)
- Clickable to seek to position
- Formula: `fillWidth = (currentTime / totalDuration) * 100%`
- Update interval: 100ms for smooth animation
- Place above or below video controls
- Inspired by jongu-tool-best-possible-self timer logic

**Task 2.11: Seek to Time Input** (Optional - 30-45 min)
- Input field for jumping to specific time
- Parse "MM:SS" format (e.g., "2:15" → 135 seconds)
- Call `youtubePlayer.seekTo(seconds)`
- Show current time / total time display
- Only implement if users request this feature
...
8. ⏳ Task 1.1: Fix import append behavior (30-60 min)
9. ⏳ Task 1.2: Update license wording (10-15 min)

**Phase 3: Recursive Landing (Enhanced UX)**
**Total Time:** ~1 hour

10. ⏳ Task 2.4: Local playlist import with Option A (1 hr)
...
11. ⏳ Task 2.5: Email collector CTA
...
### Recursive Creator
- [ ] Import YouTube playlist with existing items → appends to end
- [ ] Import Drive folder with existing items → appends to end
...
**Launch Ready:**
- ✅ CSV export (attribution)
- ✅ YouTube link visible (transparency) but not clickable (safety)
- ✅ Calendar scheduling (family planning)
- ✅ Import append working (creator UX)

**Bonus (Enhanced UX):**
- ✅ Local playlist import (try before account)
...
**Ready to start!** Let's begin with Phase 1 (Creator fixes) → Phase 2 (Landing priority 1) → Phase 3 (Local import).
